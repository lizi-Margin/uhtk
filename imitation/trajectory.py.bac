...
import time
from imitation.utils import safe_dump_traj_pool
import numpy as np
from numpy.core.numeric import indices
from .foundation import AlgorithmConfig
from imitation.traj import TRAJ_BASE
import os
import copy
import pickle
...
class trajectory(TRAJ_BASE):
    ...
    def finalize(self):
        ...
        # dead_mask = tmp.all(-1)
        dead_mask = np.zeros(shape=(self.time_pointer,), dtype=np.bool)
...
class TrajManagerBase(object):
    ...
    def batch_update(self, traj_frag):
        ...
        if isinstance(done, bool): done = np.array([done for i in range(self.n_env)])
        if isinstance(skip, bool): skip = np.array([skip for i in range(self.n_env)])
        n_active = sum(~skip)
        # feed
        cnt = 0
...
class BatchTrajManager(TrajManagerBase):
    ...
    def train_and_clear_traj_pool(self):
        ...
        # print_list(self.traj_pool) 
        current_task_l, self.traj_pool = self.pool_manager.absorb_finalize_pool(pool=self.traj_pool)
        # print亮黄("traj_pool absorbed")
        # print_list(self.traj_pool) 
        # save_container(get_container_from_traj_pool(self.traj_pool), self.update_cnt)
        # self.save_traj_pool()
        # self.load_traj_pool()
        # print_list(self.traj_pool) 
